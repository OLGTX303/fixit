...
const conversations = computed(() => groupByPerson(myBookings.value, 'provider'))
...
async function refreshConversations() {
  ...
  try {
    ...
    // Keep the current list on transient network errors; the next poll retries.
  } finally {
    polling = false
  }
}
...
function select(c) {
  ...
  else { router.push({ name: 'chat', params: { id: c.latestJob.id } }) }
}
  ...
  <div v-if="isDesktop" class="msg-split">
    ...
    <div class="msg-panel-left">
      ...
      <div class="msg-conv-list">
        <button
          v-for="c in conversations" :key="c.key"
          class="msg-conv-row"
          :class="{ active: selectedGroupKey === c.key }"
          @click="select(c)"
        >
          ...
          <div class="msg-conv-body">
            <div class="msg-conv-name">{{ c.person?.name || 'Provider' }}</div>
    ...
    <div class="d-flex flex-column gap-2">
      <button v-for="c in conversations" :key="c.key"
        ...
        <div class="flex-grow-1" style="min-width:0;text-align:left">
          <div class="fw-semibold" style="font-size:15px;margin-bottom:3px">{{ c.person?.name || 'Provider' }}</div>
